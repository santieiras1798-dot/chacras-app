// Módulo Nutrición V2: análisis simples + fertilizantes aplicados + sumatoria de nutrientes
...
loadSeason=async function(id){
  ...
  const analysisIds=(analyses||[]).map(a=>a.id);let results=[];
...
renderSeasonTab=function(tab){
  ...
  const analysisRows=(z.analyses||[]).map(a=>{const rs=(z.analysisResults||[]).filter(r=>r.analisis_id===a.id);return `<div class="card"><div class="row"><div><b>${esc(a.tipo)}</b><div class="muted">${fmtDate(a.fecha)}${a.estadio?' · '+esc(a.estadio):''}</div></div><span class="stage">Análisis</span></div>${rs.length?`<div class="analysis-results">${rs.map(r=>`<span><b>${esc(r.parametro)}</b> ${r.valor??esc(r.valor_texto||'—')}</span>`).join('')}</div>`:''}${a.observaciones?`<div class="muted">${esc(a.observaciones)}</div>`:''}</div>`}).join('');
...
async function newAnalysis(zafra_id){
  content.innerHTML=`<button class="back" onclick="openSeason('${zafra_id}','nutricion')">← Volver</button><div class="card"><h2>Nuevo análisis</h2><label>Fecha</label><input id="ana_fecha" type="date" value="${new Date().toISOString().slice(0,10)}"><label>Tipo</label><select id="ana_tipo"><option>Suelo</option><option>Planta</option><option>Tejido</option><option>Otro</option></select><label>Estado fenológico</label><input id="ana_estadio" placeholder="Ej. Z22, C1, V6"><label>Resultados</label><div id="resultRows"></div><button type="button" class="ghost full" id="addResult">+ Nutriente</button><label>Observaciones</label><textarea id="ana_obs" placeholder="Observaciones (opcional)"></textarea><button id="saveAnalysis">Guardar análisis</button><div id="status"></div></div>`;
  const rows=document.getElementById('resultRows');
  function addRow(){const d=document.createElement('div');d.className='analysis-row simple';d.innerHTML=`<input class="param" placeholder="Nutriente / parámetro"><input class="value" type="number" step="any" placeholder="Resultado">`;rows.appendChild(d)}
  ...
  document.getElementById('saveAnalysis').onclick=async()=>{
    ...
    const {data:a,error}=await db.from('analisis').insert({owner_id:user.id,zafra_id,fecha:document.getElementById('ana_fecha').value,tipo:document.getElementById('ana_tipo').value,estadio:document.getElementById('ana_estadio').value,observaciones:document.getElementById('ana_obs').value}).select().single();
    if(error){status.innerHTML=msg(error.message,'error');return}
    const resultData=[...document.querySelectorAll('.analysis-row')].map(r=>({owner_id:user.id,analisis_id:a.id,parametro:r.querySelector('.param').value.trim(),valor:r.querySelector('.value').value===''?null:+r.querySelector('.value').value})).filter(r=>r.parametro);
...
async function newNutritionFertilizer(zafra_id){
  ...
  document.getElementById('saveFert').onclick=async()=>{const s=document.getElementById('status');s.innerHTML=msg('Guardando…');const {data:labor,error}=await db.from('labores').insert({owner_id:user.id,zafra_id,fecha:document.getElementById('fert_fecha').value,tipo_labor:'fertilizacion',observaciones:document.getElementById('fert_obs').value}).select().single();if(error){s.innerHTML=msg(error.message,'error');return}const payload={labor_id:labor.id,owner_id:user.id,fertilizante:document.getElementById('fert_nombre').value,dosis_kg_ha:+document.getElementById('fert_dosis').value||0,n_kg_ha:+document.getElementById('fert_n').value||0,p2o5_kg_ha:+document.getElementById('fert_p').value||0,k2o_kg_ha:+document.getElementById('fert_k').value||0,s_kg_ha:+document.getElementById('fert_s').value||0};const r=await db.from('fertilizaciones').insert(payload);if(r.error){await db.from('labores').delete().eq('id',labor.id);s.innerHTML=msg(r.error.message,'error');return}openSeason(zafra_id,'nutricion')};
}
